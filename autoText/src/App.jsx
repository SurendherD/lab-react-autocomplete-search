...
export default function App() {
  ...
  return (
    <div>
      <h1>Search</h1>
      <input type="text" className='input'value={input} onChange={handleChange} onKeyDown={handleKeyDown} placeholder="Search..."/>
      <button>search</button>
      {showSuggestions && (<div className="suggestions">
                              {suggestions.map((suggestion, index) => (
                              <div key={index} onClick={() => handleSuggestionClick(suggestion)}className="suggestion">
                              {suggestion}
                            </div>
                          ))}
                        </div>
